...
if (isset($_POST['update_status'])) {
    ...
    // Redirect กลับไปที่เดิมเพื่อล้างค่า POST
...
if (isset($_POST['delete_reg'])) {
    ...
    // ควรลบข้อมูลการชำระเงินที่เกี่ยวข้องด้วย (ถ้ามี)
    $conn->query("DELETE FROM `การชำระเงิน` WHERE `รหัสใบสมัคร` = '$del_id'");
...
<head>
    ...
    <style>
        ...
        /* ขยายความกว้างคอลัมน์จัดการเล็กน้อย */
    </style>
</head>
                    ...
                    <div class="tab-pane fade" id="regs" role="tabpanel">
                        <div class="card card-custom border-0 shadow-sm overflow-hidden">
                            <div class="p-3 bg-white border-bottom d-flex justify-content-between align-items-center">
                                <h5 class="mb-0 fw-bold text-primary"><i class="bi bi-list-ul"></i>
                                    รายชื่อผู้สมัครล่าสุด</h5>
                                <button class="btn btn-sm btn-outline-success"><i class="bi bi-file-earmark-excel"></i>
                                    Export Excel</button>
                            </div>
                                ...
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = $r->fetch_assoc()):
                                            ...
                                            if ($row['สถานะ'] == 'รอชำระเงิน')
                                                $badgeClass = 'bg-warning text-dark';
                                            if ($row['สถานะ'] == 'รอตรวจสอบ')
                                                $badgeClass = 'bg-info text-dark'; // เพิ่มสถานะนี้
                                            if ($row['สถานะ'] == 'ชำระแล้ว')
                                                $badgeClass = 'bg-success';
                                            if ($row['สถานะ'] == 'ยกเลิก')
                                                $badgeClass = 'bg-danger';

                                            // ค้นหาสลิปการโอนเงิน (ถ้ามี)
                                            $slip_sql = "SELECT * FROM `การชำระเงิน` WHERE `รหัสใบสมัคร` = '" . $row['รหัสใบสมัคร'] . "' ORDER BY `รหัสการชำระ` DESC LIMIT 1";
                                            $slip_result = $conn->query($slip_sql);
                                            $slip_file = null;
                                            if ($slip_result->num_rows > 0) {
                                                $slip_data = $slip_result->fetch_assoc();
                                                $slip_file = $slip_data['รูปสลิป'];
                                            }
                                            ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="fw-bold">
                                                        <?= htmlspecialchars($row['ชื่อจริง']) . " " . htmlspecialchars($row['นามสกุล']) ?>
                                                    </div>
                                                    <small class="text-muted"><i class="bi bi-tag"></i>
                                                        <?= number_format($row['ราคา']) ?> บ.</small>
                                                </td>
                                                <td><span
                                                        class="badge bg-light text-dark border"><?= htmlspecialchars($row['ชื่อรายการ']) ?></span>
                                                </td>
                                                <td><span
                                                        class="badge rounded-pill <?= $badgeClass ?>"><?= $row['สถานะ'] ?></span>
                                                </td>
                                                <td>
                                                    <div class="d-flex align-items-center gap-1">
                                                        ...
                                                        <?php if ($slip_file): ?>
                                                            <a href="uploads/slips/<?= $slip_file ?>" target="_blank"
                                                                class="btn btn-sm btn-info text-white" title="ดูสลิป">
                                                                <i class="bi bi-receipt"></i>
                                                            </a>
                                                        <?php else: ?>
                                                            <button class="btn btn-sm btn-light text-muted" disabled
                                                                title="ไม่มีสลิป"><i class="bi bi-receipt"></i></button>
                                                        <?php endif; ?>

                                                        <form method="POST" action="" class="d-flex">
                                                            <input type="hidden" name="reg_id"
                                                                value="<?= $row['รหัสใบสมัคร'] ?>">
                                                            <input type="hidden" name="update_status" value="1">

                                                            <select name="status" class="form-select form-select-sm ms-1"
                                                                style="width: 110px; font-size: 0.8rem;">
                                                                <option value="รอชำระเงิน" <?= $row['สถานะ'] == 'รอชำระเงิน' ? 'selected' : '' ?>>รอโอน</option>
                                                                <option value="รอตรวจสอบ" <?= $row['สถานะ'] == 'รอตรวจสอบ' ? 'selected' : '' ?>>รอตรวจสอบ</option>
                                                            ...
                                                            <button type="submit"
                                                                class="btn btn-sm btn-outline-primary ms-1" title="บันทึก">
                                                                <i class="bi bi-check-lg"></i>
                                                            </button>
                                                        </form>

                                                        <form method="POST" action="" class="form-delete ms-1">
                                                            <input type="hidden" name="reg_id"
                                                                value="<?= $row['รหัสใบสมัคร'] ?>">
                        ...
                        <div class="card card-custom border-0 p-4 mb-4">
                            ...
                            <form action="api_add_race.php" method="POST" class="row g-2">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="name" class="form-control" placeholder="เช่น Fun Run 5KM"
                                        required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label text-muted small">ระยะทาง (กม.)</label>
                                    <input type="number" step="0.1" name="dist" class="form-control" placeholder="0.0"
                                        required>
                                </div>
                                <div class="col-md-2 d-flex align-items-end">
                                    <button class="btn btn-primary w-100"><i class="bi bi-plus-circle me-1"></i>
                                        บันทึก</button>
                                </div>
                            </form>
                        </div>
